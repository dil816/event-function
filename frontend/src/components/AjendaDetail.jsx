...
const AjendaDetail = ({ ajend }) => {
  ...
  return (
    <div className="bg-[#fff] w-auto h-[120px] rounded-[20px] mx-[auto] my-[20px] p-[15px] relative [box-shadow:2px_2px_5px_rgba(0,_0,_0,_0.05)]">
      ...
      <h4 className="m-0 px-[6px] py-[0] text-[1em] text-[#555]">{ajend.title}</h4>
      <span className="left-[40px] px-[6px] py-[0] text-[#333] text-[0.8em]">{ajend.timeRange} min</span>
      <button className="absolute top-[20px] right-[20px] w-[70px] cursor-pointer bg-[#fff] p-[6px] text-[rgb(19,_241,_30)] border-[2px] border-[solid] border-[rgb(19,241,30)] rounded-[20px] " onClick={handleUpdate}>
        Edit
      </button>
      <button className="absolute bottom-[10px] right-[20px] w-[70px] cursor-pointer bg-[#fff] p-[6px] text-[rgb(226,_43,_43)] border-[2px] border-[solid] border-[rgb(226,43,43)] rounded-[20px]" onClick={handleDelete}>
        Delete
      </button>
    </div>
  );
